...
import { fetchAsynchomecomputers } from '../../Redux/homeSlice';
import { getHomecomputer } from '../../Redux/homeSlice';
import { useDispatch,useSelector } from 'react-redux';
import { thumbimgURL } from '../../Utils/Api/Imageapi';
import { getCountry } from '../../Redux/countrySlice';
import { useTranslation } from 'react-i18next';

function Categorycomputer() {
  const { t, i18n } = useTranslation();
    ...
    useEffect(() => {
     dispatch(fetchAsynchomecomputers(country))
    }, [country])
    const homecomputer=useSelector(getHomecomputer)
    console.log('homecomputer',homecomputer);
    ...
    <div>
            <div className='bestdeals'>
                <div className='bestdeals-head'>
                    <h3 className='pe-5'>{t("Computer Collections")}</h3>
                </div>
